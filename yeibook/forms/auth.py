...
class RegisterForm(FlaskForm):
	...
	password = PasswordField(validators=[DataRequired(message='密码不得为空'), Length(6, 32, message='密码需要6-32位喔')])
	confirm_password = PasswordField(validators=[DataRequired(message='密码不得为空'), Length(6, 32, message='密码需要6-32位喔')])
	wx_avatar = StringField()
...
class EmailForm(FlaskForm):
	email = StringField(validators=[DataRequired('邮箱帐号不得为空喔'), Length(8, 64),
